...
func (element *ConfigST) createCodecsFromCameraURL(streamName string, cameraURL string) []av.CodecData {
	// Initialize the slice to hold codec data
	codecs := []av.CodecData{}

	// Here you would normally fetch and parse the RTSP stream.
	// For illustration, let's assume we get the H.264 codec data from the stream.

	// Simulate fetching H.264 codec data (replace this with actual parsing logic)
	codec := h264parser.CodecData{}
	if codec.SPS() == nil || codec.PPS() == nil {
		log.Println("No SPS/PPS found in stream, waiting...")
		time.Sleep(50 * time.Millisecond)
		// Retry or handle the issue here
	} else {
		// If SPS/PPS is valid, append the codec to the list
		codecs = append(codecs, codec)
	}

	// Return the generated codec list
	return nil
}
